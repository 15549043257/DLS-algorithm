...
if __name__ == "__main__":
    #导入参数----------------------------------------------------------------------
    tao = []
    with open(r"F:\everything\master\study\实验数据\变压器DLS\230525\362.2+806.9PS1比1\1.fit", 'r',
        ...
        for row in reader:
            i += 1
            if i > 4:
                tao.append(round(float(row[0]) * 10 ** (-6), 7))
    tao=np.array(tao).reshape(1,-1)

    lamda = 532E-9#
    nm = 1.333
    theta = np.pi/180*90
    q=4*np.pi*nm/lamda*np.sin(theta/2)
    Kb=1.38E-23
    T=273.15+25.9
    eta=0.873E-3
    # 创建一个随机向量函数链神经网络--------------------------------------------------------
    ...
    hidden_size = 100
    output_size = 1
    model = RVFLNN(input_size, hidden_size, output_size)
    # 生成随机训练数据------------------------------------------------------
    num_training_set=11
    noise_level=0.001
